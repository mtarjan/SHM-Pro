...
eos.conus <- eos %>% filter(!(SUBNATION_CODE %in% c("AK", "HI")) & NATION == "United States" & NUM_CURRENT_EOS > 0) %>% group_by(ELEMENT_GLOBAL_ID) %>% summarise(NUM_CURRENT_EOS = sum(NUM_CURRENT_EOS), EO_STATES = paste0(SUBNATION_CODE, collapse = ", "))
...
qry <- "SELECT DISTINCT egt.element_global_id, gname.scientific_name, egt.g_primary_common_name, nc.name_category_desc, egt.rounded_g_rank, ESA_TG.D_USESA_ID, d_usesa.display_value USESA
...
LEFT JOIN d_usesa
  ON esa_tg.d_usesa_id = d_usesa.d_usesa_id
WHERE
...
## CONUS - no AK or HI (EOs only reported for CONUS)
cand <- dat %>%
...
## Get list of species for permissions
